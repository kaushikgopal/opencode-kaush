#!/usr/bin/env bash
# Publish an opencode-kaush plugin to npm via GitHub Actions OIDC.
#
# Usage:
...
if [ -z "$PACKAGE" ]; then
  echo "Usage: make publish PACKAGE=<plugin-name> [VERSION=<x.y.z>]" >&2
  echo "Known packages: opencode-intercom" >&2
  exit 1
fi
